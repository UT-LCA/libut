...
static pthread_attr_t controlattr;
static int controlfd;
static int clientfds[HWALLOCD_MAX_PROC];
static struct proc *clients[HWALLOCD_MAX_PROC];
        ...
        int *fds, int n_fds)
{
    ...
    for (i = 0; i < hdr.thread_count; i++) {
        ...
        /* attach the TX queue */
        ret = shm_init_lrpc_in(&reg, &s->txq, &th->txq);
        if (ret)
            goto fail_free_proc;
...
static void control_add_client(void)
{
    ...
    if (nr_clients >= HWALLOCD_MAX_PROC) {
...
static void *control_loop(void *data)
{
    fd_set readset;
    int maxfd, i, nrdy;
    uint64_t cmd;
    ...
    while (1) {
        ...
        while (lrpc_recv(&lrpc_data_to_control, &cmd, &payload)) {
            ...
            assert(cmd == CONTROLPLANE_REMOVE_CLIENT);
...
int control_init(void)
{
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(core_assign.ctrl_core, &cpuset);
    pthread_attr_setaffinity_np(&controlattr, sizeof(cpuset), &cpuset);
    if (pthread_create(&tid, &controlattr, control_loop, NULL) == -1) {
